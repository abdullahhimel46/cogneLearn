 ...
 *
 * <p>
 * {@code @RestControllerAdvice} means Spring intercepts every exception thrown
 * in any
 * {@code @RestController} and lets us decide what HTTP response to send.
 * Without this, Spring would send back HTML error pages (not useful for an
 * API).
 *
 * <p>
 * This class replaces the old JavaScript try/catch blocks:
 * 
 * <pre>
 ...
 * <li>403 Forbidden â€” the client IS logged in but doesn't have
 * permission</li>
 * <li>404 Not Found â€” the requested resource doesn't exist</li>
 * <li>500 Internal Server Error â€” something unexpected went wrong on the
 * server</li>
 * </ul>
 */
...
public class RestExceptionHandler {
        ...
        /**
         * Handle illegal argument errors (e.g., "Playlist not found", "Email already
         * exists").
         *
         * <p>
         * These are thrown explicitly in our service classes when the data is correct
         * in format
        ...
        /**
         * Handle unexpected server-side errors (e.g., "User disappeared after
         * authentication").
         *
         * <p>
         * These are thrown when something unexpected happens on the server side
        ...
        /**
         * Handle Spring Security authentication failures (wrong password, account not
         * found).
         *
         * <p>
         * Maps to JS:
         * {@code if (!Auth.login(email, password)) { alert("Invalid credentials"); }}
        ...
        public ResponseEntity<ApiError> handleAuthenticationException(AuthenticationException ex) {
                // Keep login failures generic, but return a clearer message for missing
                // sessions.
         ...
         *
         * <p>
         * When a field fails validation (e.g., email is blank, password too short),
         * Spring throws this exception. We collect all field errors into one message.
         *
         * <p>
         * Maps to JS: form validation like:
        ...
        /**
         * Handle constraint violations from {@code @Validated} or path variable
         * validation.
         *
         ...
         *
         * <p>
         * Example: calling {@code /api/v1/proxy/playlist} without {@code ?playlistId=}
         * returns
         ...
         *
         * <p>
         * This prevents stack traces from leaking to the API client.
         * Instead, we log the error internally (future work) and return a generic 500
         * message.
         *
